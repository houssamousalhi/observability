...
def get_grafana_api_key():
    ...
    try:
        ...
        response = secretsmanager.get_secret_value(SecretId=api_key_path)
...
def update_grafana_datasource(access_key, secret_key, datasource_name):
    ...
    try:
        ...
        logger.info(
            f"Updating Grafana datasource '{datasource_name}' at URL: {grafana_url}"
        )
        ...
        cloudwatch_ds = next(
            (
                ...
                if ds["type"] == "cloudwatch" and ds["name"] == datasource_name
            ),
            None,
        )
        ...
        if not cloudwatch_ds:
            logger.error(
                f"CloudWatch datasource with name '{datasource_name}' not found in Grafana"
            )
            return
        ...
        update_data = {
            ...
            "secureJsonData": {"accessKey": access_key, "secretKey": secret_key},
        }
        ...
        logger.info(
            f"Grafana API response: status={response.status_code}, content={response.text}"
        )
        response.raise_for_status()

        logger.info(
            f"Successfully updated Grafana CloudWatch datasource '{datasource_name}'"
        )
...
def rotate_access_key(target_username, rotation_period, datasource_name):
    ...
    try:
        ...
        try:
            ...
            return {"statusCode": 404, "body": f"User {target_username} does not exist"}

        # If no access keys exist, create one
        if not access_keys["AccessKeyMetadata"]:
            logger.info(
                f"No access keys found for user {target_username}, creating new key"
            )
            ...
            update_grafana_datasource(
                new_key["AccessKeyId"], new_key["SecretAccessKey"], datasource_name
            )
        ...
        for key in access_keys["AccessKeyMetadata"]:
            ...
            if key_age > rotation_period and status == "Active":
                logger.info(
                    f"Key {key_id} needs rotation (older than {rotation_period} days)"
                )

                # First deactivate the old key
                iam.update_access_key(
                    UserName=target_username, AccessKeyId=key_id, Status="Inactive"
                )
                logger.info(f"Key {key_id} deactivated")

                # Delete the old key
                iam.delete_access_key(UserName=target_username, AccessKeyId=key_id)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(
            "Starting with configuration: rotation_period=%d days, target_username=%s, target_datasource=%s",
            rotation_period,
            target_username,
            datasource_name,
        )
