...
def get_resource_info(alarm):
    ...
    for dimension in dimensions:
        if dimension['Value']:  # Only add non-empty values
            ...
            if dimension['Name'] in ['QueueName', 'FunctionName', 'InstanceId', 'DBInstanceIdentifier', 
                                   'LoadBalancerName', 'AutoScalingGroupName', 'TopicName',
                                   'TableName', 'BucketName', 'ApiId', 'ApiName',
                                   'StateMachineArn', 'ExecutionId', 'ActivityArn']:
...
def count_alarms_by_resource_type(alarms):
    ...
    resource_type_counts = defaultdict(lambda: {
        'alarm_count': 0,
        'ok_count': 0,
