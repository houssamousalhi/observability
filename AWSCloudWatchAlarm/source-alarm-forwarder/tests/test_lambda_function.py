...
def test_put_resource_metrics(mock_cloudwatch):
    ...
    alarm_metric = next(m for m in metric_data if m['MetricName'] == 'ResourceTypeAlarmsInAlarmState')
    ok_metric = next(m for m in metric_data if m['MetricName'] == 'ResourceTypeAlarmsInOkState')
    insufficient_metric = next(m for m in metric_data if m['MetricName'] == 'ResourceTypeAlarmsInInsufficientState')
